...
* поддержка синтаксиса различными средами разработки и инструментами (т.к. код -- обычный JavaScript):
	* подсветка кода;
	* JSHint, JSLint, etc. 
* может выполняться тот же код, который пишется (в [dev-среде](#runmode)). Это упрощает поиск ошибок и отладку;
* более высокая скорость [компиляции](#run) (особенно в dev-среде).
...
Код JavaScript, полученный на этапе выполнения шаблонов, выполняется. Ход выполнения одинаков для всех синтаксисов и режимов исполнения:
